        ...
        function wrapChars(node) {
            ...
            childNodes.forEach(child => {
                if (child.nodeType === Node.TEXT_NODE) {
                    // Split the text into words and spaces using regex
                    const words = child.textContent.split(/(\s+)/);
                    
                    words.forEach(word => {
                        if (word.trim() === '') {
                            // Preserve the spaces outside the spans
                            html += word; 
                        } else {
                            // Wrap the entire word to prevent mid-word breaking
                            html += `<span class="word" style="display: inline-block; white-space: nowrap;">`;
                            
                            // Now split into characters inside the word container
                            const chars = word.split('');
                            chars.forEach(char => {
                                // Transforms require inline-block to render properly
                                html += `<span class="scroll-char" style="display: inline-block;">${char}</span>`;
                            });
